...
if(isset($_GET["userName"]))
{
	$user=$_GET["userName"];
		...
		$sql="SELECT * FROM student_register WHERE Email='$user'";
		$ses_result=mysql_query($sql);        
       	$count=mysql_num_rows($ses_result);
		
		//Checking for recruiter table
		$ses_result1=mysql_select_db($dbname) or die(mysql_error());
		$sql1="SELECT * FROM recruiter_register WHERE Email='$user'";
...
if(isset($_GET['User']))
{
	$Userlogin=$_GET['User'];
		...
		$ses_result=mysql_select_db($dbname) or die(mysql_error());
		$sql="SELECT * FROM student_register WHERE UserName='$Userlogin'";
		$ses_result=mysql_query($sql);        
       	$count=mysql_num_rows($ses_result);
