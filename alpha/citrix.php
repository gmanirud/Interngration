...
class Citrix
{
	//protected $organizerKey="384723869081067013";
    //protected $accessToken="4f8530d951f7963ce4180253c531c3f6";
	
	protected $organizerKey="3958452219837304838";	
	protected $accessToken="e1a64adaabd4833f31344cd1fd6a5b6b";
	...
	*/
	...
	protected $access_token = "4f8530d951f7963ce4180253c531c3f6";
	...
	*/
	public function get_organizer_key() 
	{
		...
		//{
			...
			{
				//$code = $_GET['code'];
				$code = "MTc4OGIwMmY6MTQwY2ZjZWUwMDE6LTNlMWM=";
				//$access_token = $this->get_access_token($code);
				$access_token = "4f8530d951f7963ce4180253c531c3f6";
				if ($access_token) 
				{
				  //return $this->organizer_key;
				  return "1866916119511197701";
				}
			}
		//}
	...
	*/
	function auth_citrixonline($redirect_url = '') 
    {
		...
		return 'https://api.citrixonline.com/oauth/authorize?client_id=' . $this->citrix_api_key . '&redirect_uri=http://www.keshavatech.com/interngration/student-homepage.php';
		...
		//$params[CURLOPT_HTTPHEADER] = array('Accept: application/json', 'Content-Type: application/json', 'Authorization: OAuth oauth_token='.$this->access_token);
			...
			//$reponse = json_decode($this->make_request("https://api.citrixonline.com/oauth/access_token?grant_type=password&user_id=anirudh.gm@mail.utoronto.ca&password=Keshava1&client_id=2a3328f6717227aa1262eb368d013a47", $params), true);
	...
	*/	
	...
	function citrixonline_get_list_of_webinars() 
    {
		//if(!$this->organizer_key or !$this->access_token)
		//	return 0;
		...
		$reponse = json_decode($this->make_request("https://api.citrixonline.com/G2W/rest/organizers/".$this->organizerKey."/upcomingWebinars?oauth_token=".$this->accessToken), true);
		...
		if($type>0)
		{
			...
			//  Insert the data from gotomeeting to our local DB
                       $weblist=$return_array;
						
						$cnt=count($weblist["upcoming"]["webinars"]);
						
						for($i=0;$i<=$cnt;$i++)
						{
						$desc=str_replace("'","\'",$weblist["upcoming"]["webinars"][$i]["description"]);
						$subj=str_replace("'","\'",$weblist["upcoming"]["webinars"][$i]["subject"]);
						...
						if(isset($webkey)!=0)
						{
							...
							$res1=mysql_fetch_array($ses_result);
							    $subject=$res1["subject"];
								$description=$res1["description"];
								$webinar_url=$res1["webinar_url"];
								$webinar_key=$res1["webinar_key"];
								$start_time=$res1["start_time"];
								$end_time=$res1["end_time"];
							...
							if($rowcount=='0')
							{
								...
								//$sqlInserttbl = "insert into registered_webinar (subject,description,webinar_url,webinar_key,start_time,end_time) values ('$subj','$desc','$regurl','$webkey','$starttime','$endtime')";
								//$resulttbl=mysql_db_query($dbname,$sqlInserttbl,$link);
								
							}
							else
							{
								
								if(($subject!=$subj)||($description!=$desc)||($start_time!=$starttime)||($end_time!=$endtime))
								{
									
									$sqlUpdate ="Update upcoming_webinar set subject='$subj', description='$desc', start_time='$starttime', end_time='$endtime'  where webinar_key='$webkey'";
									
									$result1=mysql_db_query($dbname,$sqlUpdate,$link);
									
									$sqlUpdate12 ="Update student_web_reg set subject='$subj', description='$desc', starttime='$starttime', endtime='$endtime'  where webinar_id='$webkey'";
									$result1=mysql_db_query($dbname,$sqlUpdate12,$link);
								}
						...
						// Update the Cancellation to Local DB while Cancelled in to gotomeeting webinar
						
						        $chk='0';
								$sqlget12="Select *  from upcoming_webinar where start_time>NOW()";
								$ses_result12=mysql_query($sqlget12);
								while($res12=mysql_fetch_array($ses_result12))
								{
							    $subject=$res12["subject"];
								$description=$res12["description"];
								$webinar_url=$res12["webinar_url"];
								$webinar_key=$res12["webinar_key"];
								$start_time=$res12["start_time"];
								$end_time=$res12["end_time"];
								for($i=0;$i<=$cnt;$i++)
								{
								$desc=str_replace("'","\'",$weblist["upcoming"]["webinars"][$i]["description"]);
								$subj=str_replace("'","\'",$weblist["upcoming"]["webinars"][$i]["subject"]);
								...
								if($chk=='1')
								{
									//$sqlUpdates ="Update upcoming_webinar set status='Cancelled'  where webinar_key='$webinar_key'";
									//$result1=mysql_db_query($dbname,$sqlUpdates,$link);
									
									//$sqlUpdatesstudent ="Update student_web_reg set status='Cancelled'  where webinar_id='$webinar_key'";
									//$result1=mysql_db_query($dbname,$sqlUpdatesstudent,$link);
									
									
									
									$sqldelete ="delete from upcoming_webinar  where webinar_key='$webinar_key'";
									$result1=mysql_db_query($dbname,$sqldelete,$link);
									
									
									$sqldeletestudent ="delete from student_web_reg where webinar_id='$webinar_key'";
									$result1=mysql_db_query($dbname,$sqldeletestudent,$link);
									
									//print $sqldeletestudent;
	...
	*/
    function citrixonline_create_registrant_of_webinar($webinar_id=false, $data = array(), $all_fields_chk = false) 
    {
		...
		//{
			...
			$reponse = json_decode($this->make_request("https://api.citrixonline.com/G2W/rest/organizers/".$this->organizerKey."/webinars/".$webinar_id."/registrants", $params), true);
